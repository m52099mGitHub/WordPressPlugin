...
if (current_user_can('manage_options') && isset($_POST['submit-dashboard-for-different-login-user-page']) && wp_verify_nonce(sanitize_text_field($_POST['dashboard_for_different_login_user_nonce_field']), 'dashboardfordifferentloginuser_action')) {
    ...
    //存储表单提交的内容（$setdisplayitemforuseroptions）：以名称"'当前输入的用户名'_set_display_item_for_user_options"的形式存储到数据库options表中,并返回布尔值
    ...
    if (username_exists($currentInputUserName)) {
        echo "<p style='text-align: center; color: #000; '>您输入的用户: ".$currentInputUserName." 已在数据库中,新权限设置成功!</p>";
    }
    else {
        echo "<p style='text-align: center; color:#E11;'>您输入的用户: ".$currentInputUserName." 不在数据库中,不具备授权条件!</p>";
    }
        ...
        <div id="config-box" class="config-box">
            <div id="form-content">
                <form action="" method="post" name="dashboard_for_different_login_user_form">
                    <?php
                    // nonce 字段用于验证表单的内容是否来自当前站点上的位置，而不是其他地方。nonce 不提供绝对保护，但应该可以防止大多数情况。在表单中使用 nonce 字段非常重要。
                    ...
                    <table id="form-table">
                        <tr>
                            ...
                            <td>
                                <?php 
                                    //从数据库获取用户的菜单配置信息
                                    $opt = get_option($currentInputUserName.'_set_display_item_for_user_options'); 
                                ?>
                                <!-- <input id="userRole" name="userRole" type="text" value="<?php #echo $opt['userRole']; ?>"> -->
                                <input id="userRole" name="userRole" type="text" value="<?php echo $currentInputUserName; ?>">
                            </td>
                        </tr>
                        <tr>
                            <th>
