...
/**
 * Plugin Name:Dashboard for Different Login User Testing Copy
    ...
    class dashboard_menu_adjust
    {
        ...
        public function dashboard_menu_display()
        {
            ...
            if (username_exists($userNameInput)) {
                ...
                // echo '<p style="text-align:center; color:#E66;">用户:' . $userNameInput . '已注册, 他是' .$userLevel. '级用户, ID为' . $userId .', 已被授予新的权限</p>';
                
                if ($currentUserName  == $userNameInput) { //若当前用户公开显示的名称是Colin,则不执行后续语句。若不是则对管理员用户Admin和Colin以外的以外的用户隐藏下面的菜单内容
        ...
        public function load_dashboard_menu_when_user_login () {
            ...
            print_r('当前登录的用户名为: '.$currentUserName);
