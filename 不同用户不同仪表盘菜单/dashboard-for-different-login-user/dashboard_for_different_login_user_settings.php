...
if (current_user_can('manage_options') && isset($_POST['submit-dashboard-for-different-login-user-page']) && wp_verify_nonce(sanitize_text_field($_POST['dashboard_for_different_login_user_nonce_field']), 'dashboardfordifferentloginuser_action')) {
    ...
    // print_r($setdisplayitemforuser);
    //存储表单获取的内容,返回布尔值
    $saveSettings = update_option('set_display_item_for_user_options', $setdisplayitemforuseroptions);
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if (username_exists($userNameInput)) {
                                    ...
                                    echo '<p id="tip" style="width: 150px; color:red;text-align:right; margin-left:350px;">' . $userNameInput . '已注册, 他是' . $userRole . '用户, ID为' . $userId . ', 可以向他授权可查看菜单</p>';
                                } else {
                                    echo '<p id="tip" style="width: 150px; color:red;text-align:right; margin-left:350px;">' . $userNameInput . '不是注册用户,不可以向他授权可查看菜单</p>';
                                }
                                ?>
                            </td>
                        </tr>
